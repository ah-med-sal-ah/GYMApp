...
const navItems = [
  { name: 'admin.users.index', label: 'Users', icon: '🏢' },
  { name: 'admin.profile', label: 'Profile', icon: '👤' },
]

onMounted(() => {
  if (!adminAuth.admin) {
    adminAuth.fetchProfile().catch(() => {})
  }
})
...
<template>
  <div class="flex min-h-full">
    ...
    <aside
      class="fixed inset-y-0 left-0 z-30 w-64 -translate-x-full bg-slate-950 text-slate-200 transition-transform lg:static lg:translate-x-0"
    ...
    >
      ...
      <div class="absolute bottom-0 w-64 border-t border-slate-800 p-3">
        <button
          type="button"
          class="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-slate-300 transition hover:bg-slate-800 hover:text-white"
          @click="logout"
        >
          <span>🚪</span> Logout
        </button>
